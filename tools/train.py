...
def parse_config():
    ...
    parser.add_argument('--eval_freq', type=int, default=1, help='evaluation frequency during training')
...
def main():
    ...
    teacher = build_vision_network(model_cfg=cfg.MODEL, num_class=len(cfg.CLASS_NAMES), dataset=train_set)
    ...
    if args.pretrained_model is not None:
        teacher.load_params_from_file(filename=args.pretrained_model, to_cpu=dist_train, logger=logger)
    ...
    # for i in novel_class_idx: text_embed[i] = 0
    ...
    if dist_train:
        ...
        teacher = nn.parallel.DistributedDataParallel(
            teacher, device_ids=[cfg.LOCAL_RANK % torch.cuda.device_count()],
            find_unused_parameters=args.find_unused_parameters)
    #logger.info(model)
    ...
    #text_embed[5]=text_embed[9]=text_embed[12]=text_embed[16]=0
    train_model(
        args,
        model,
        teacher,
