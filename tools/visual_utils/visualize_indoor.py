...
def get_input(opt):
    ...
    if opt.dataset in ['scannet', 'scannet200']:
        input_file = os.path.join(opt.data_root, opt.room_name +'/'+opt.room_name + '_vh_clean_2.ply')
        label_file = os.path.join('D:/PycharmProjects/PLA-main/data/scannetv2', '{}'.format(opt.split), opt.room_name + '.pth')
        print(label_file)
