...
def eval_one_epoch(cfg, args, model, optimizer,accumulated_iter,dataloader, text_embed,epoch_id, logger, dist_test=False, writer=None,
                   best_metric=0.0, best_epoch=-1, task='sem', eval_output_dir=None):
    if task == 'sem':
        return eval_sem(
            cfg, args, model, optimizer,accumulated_iter,dataloader,text_embed,epoch_id, logger, dist_test, writer, best_metric,
...
def eval_sem(cfg, args, model, optimizer,accumulated_iter,dataloader, text_embed,epoch_id, logger, dist_test=False, writer=None,
    ...
    for i, batch_dict in enumerate(dataloader):
        ...
        batch_dict['text_embed']=text_embed
        with torch.no_grad():
    ...
    if best_metric is not None:
        ...
        if metric > best_metric:
            best_metric = metric
            best_epoch = epoch_id
            ckpt_dir = output_dir / 'bestckpt'
            ckpt_dir.mkdir(parents=True, exist_ok=True)
            ckpt_list = glob.glob(str(ckpt_dir / 'checkpoint_epoch_*.pth'))

            if ckpt_list.__len__() >= 1:
                #ckpt_list.sort(key=os.path.getmtime)
                for cur_file_idx in range(0, len(ckpt_list)):
                    os.remove(ckpt_list[cur_file_idx])

            ckpt_name = ckpt_dir / ('checkpoint_epoch_%d' % best_epoch)
            save_checkpoint(
                checkpoint_state(model, optimizer, epoch_id, accumulated_iter, best_metric, best_epoch),
                filename=ckpt_name,
            )
            #arnold.save_ckpt(str(ckpt_name) + '.pth', last_epoch=True)

        # if metric>0.01:
        #     ckpt_dir1 = output_dir / 'ckpt'
        #     ckpt_dir1.mkdir(parents=True, exist_ok=True)
        #     #ckpt_list1 = glob.glob(str(ckpt_dir1 / 'checkpoint_epoch_*.pth'))
        #     # if ckpt_list1.__len__() >= 1:
        #     #     #ckpt_list.sort(key=os.path.getmtime)
        #     #     for cur_file_idx in range(0, len(ckpt_list1)):
        #     #         os.remove(ckpt_list1[cur_file_idx])
        #     ckpt_name1 = ckpt_dir1 / ('checkpoint_epoch_%d' % epoch_id)
        #     save_checkpoint(
        #         checkpoint_state(model, optimizer, epoch_id, accumulated_iter, best_metric, best_epoch),
        #         filename=ckpt_name1,
        #     )
    ...
    return best_metric, best_epoch, metric
