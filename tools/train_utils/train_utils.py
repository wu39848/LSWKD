...
def train_one_epoch(args, model, teacher,optimizer, train_loader, text_embed,lr_scheduler, accumulated_iter, rank,
    ...
    for cur_it in range(total_it_each_epoch):
        ...
        intersection_meter, union_meter, target_meter, output_meter, accuracy = common_utils.update_meter(
            intersection_meter, union_meter, target_meter, output_meter, ret_dict2['seg_preds'],
            ret_dict2['seg_labels'], num_train_class
        )

        # record evaluation metrics for binary head
        if cfg.MODEL.get('BINARY_HEAD', None) and ret_dict2['binary_preds'] is not None:
            binary_preds = ret_dict2['binary_preds']
            binary_intersection_meter, binary_target_meter = common_utils.update_binary_acc_meter(
                binary_intersection_meter, binary_target_meter, binary_preds, ret_dict2['seg_labels'],
...
def train_model(args, model, teacher,optimizer, train_loader, val_loader, text_embed,lr_scheduler, optim_cfg, start_epoch, start_iter,
    ...
    for cur_epoch in range(start_epoch, args.epochs):
        ...
        accumulated_iter = train_one_epoch(
            args, model, teacher,optimizer, train_loader,text_embed,
        ...
        best_metric, best_epoch,metric = eval_utils.eval_one_epoch(
            cfg, args, model, optimizer,accumulated_iter,val_loader, text_embed,trained_epoch, logger, dist_train, tb_log, best_metric, best_epoch, task
        )
        if metric>0.01:
            save_trained_model(
                model, optimizer, accumulated_iter, trained_epoch, args.epochs, args.ckpt_save_interval, rank,
