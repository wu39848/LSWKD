...
def eval_single_ckpt(model, test_loader, args, text_encoder, text_embed, logger, epoch_id, dist_test=False, eval_output_dir=None):
    ...
    eval_utils.eval_one_epoch(
        cfg, args, model, None,None,test_loader,text_embed, epoch_id, logger, dist_test=dist_test, writer=None,
        best_metric=None, best_epoch=None, task=task, eval_output_dir=eval_output_dir
    )
...
def repeat_eval_ckpt(model, test_loader, args, eval_output_dir, logger, ckpt_dir, dist_test=False):
    ...
    while True:
        ...
        model.load_params_from_file(filename=cur_ckpt, logger=logger, epoch_id=cur_epoch_id, to_cpu=dist_test)
        model.cuda()
...
def main():
    ...
    if args.batch_size is None:
        args.batch_size = cfg.OPTIMIZATION.get('TEST_BATCH_SIZE_PER_GPU', cfg.OPTIMIZATION.BATCH_SIZE_PER_GPU)
    else:
