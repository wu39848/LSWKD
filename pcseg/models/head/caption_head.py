...
class CaptionHead(nn.Module):
    ...
    def forward_given_type_caption(self, batch_dict, caption_info, adapter_feats, logit_scale):
        ...
        pooled_logits, real_n_points, if_has_pts = caption_func(
            batch_dict, caption_info, adapter_feats,logit_scale
        )
        # if some scene don't have suitable image, len(pooled_features) == 0
        if len(pooled_logits) > 0:
            pooled_logits = torch.cat(pooled_logits, 0)
            ...
            caption_logit, caption_labels = self.prepare_caption_loss_logit_and_labels(
                pooled_logits, normed_caption_embed, logit_scale, caption_idx, exist_caption_idx
            )
            ret_dict = {'caption_output': caption_logit, 'caption_labels': caption_labels,
    ...
    def _forward_given_type_caption(self, batch_dict, caption_info, adapter_feats,caption_logit_scale):
        ...
        for b in range(len(frame_corr_idx)):
            ...
            for i, idx in enumerate(_frame_corr_idx):
                ...
                if selected_mask.sum() > 0:
                    ...
                    if self.feat_norm:
                        _pooled_feats = nn.functional.normalize(_pooled_feats, dim=-1)
                    caption_logit=_pooled_feats @ normed_caption_embed.float().T*caption_logit_scale
                    caption_logit=F.log_softmax(caption_logit,-1)
                    caption_logit = torch.mean(caption_logit, 0,keepdim=True)
                    pooled_feats.append(caption_logit)
            if_has_pts.append(batch_if_has_pts)

        return pooled_feats, real_n_points, if_has_pts

    def prepare_caption_loss_logit_and_labels(self, pooled_logits, caption_embed,
        ...
        if loss_type == 'CrossEntropy':
            ...
            #pooled_logits=pooled_logits*caption_logit_scale
        ...
        return pooled_logits, caption_labels
