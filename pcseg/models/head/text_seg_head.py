...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, y):
        ...
        # 计算加权和
        #V = self.value(y)
        output = torch.matmul(attn_weights, y)
...
class TextSegHead(nn.Module):
    def __init__(self, model_cfg, in_channel, ignore_label, **kwargs):
        ...
        self.cross_attn_layer=CrossAttention(512,512)
        # create cls head
    ...
    def forward(self, batch_dict):
        ...
        #     return batch_dict
        #text_embed=batch_dict['text_embed']
        ...
        # adapter_feats = adapter_feats.cuda()
        # adapter_feats=adapter_feats.unsqueeze(0)
        # text_embed=text_embed.unsqueeze(0)
        # new=adapter_feats.clone()
        # if not self.training:
        #     binary_preds = batch_dict['binary_ret_dict']['binary_preds']
        #     cur=0
        #     for i in new:
        #         i = i.unsqueeze(0)
        #         text_embed1 = text_embed.clone()
        #         if binary_preds[cur].item()==1:
        #             for j in range(20):
        #                 if j in self.novel_class_idx: text_embed1[j]=0
        #         else:
        #             for j in range(20):
        #                 if j in self.base_class_idx: text_embed1[j]=0
        #         text_embed1 = text_embed1.unsqueeze(0)
        #
        #         new[cur]=self.cross_attn_layer(i, text_embed).squeeze(0)
        #         cur+=1
        # adapter_feats = adapter_feats+new
        # adapter_feats=adapter_feats+self.cross_attn_layer(adapter_feats, text_embed).squeeze(0)
        # adapter_feats = adapter_feats.squeeze(0)
        ...
        self.forward_ret_dict['adapter_feats'] = adapter_feats
    ...
    def correct_seg_pred_with_binary_pred(self, batch_dict, semantic_scores):
        ...
        # cur=0
        # for i in semantic_scores:
        #     if binary_preds[cur].item()==1: i[..., self.novel_class_idx]=0
        #     else: i[..., self.base_class_idx]=0
        #     cur+=1
